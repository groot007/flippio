...
/**
 * Hook for fetching table data from the currently selected database
 * @param tableName - Name of the table to fetch data from
 * @returns Query result with table columns and rows
 */
export function useTableDataQuery(tableName: string) {
  ...
  return useQuery({
    queryKey: tableName && selectedDatabaseFile?.path
      ? databaseQueryKeys.tableDataQuery(tableName, selectedDatabaseFile.path)
      : databaseQueryKeys.tableData(),
    queryFn: async () => {
      if (!tableName) {

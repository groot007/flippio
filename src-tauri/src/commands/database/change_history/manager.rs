...
impl ChangeHistoryManager {
    ...
    pub async fn record_change(&self, change: ChangeEvent) -> Result<(), String> {
        ...
        if changes_map.len() >= self.max_total_contexts
            && !changes_map.contains_key(&change.context_key)
        {
        ...
        while context_changes.len() >= self.max_changes_per_context {
            ...
            if let Some(removed_change) = removed {
                log::debug!(
                    "Removed oldest change: {} from context: {}",
                    removed_change.id,
                    change.context_key
                );
            }
    ...
    pub async fn get_changes_for_table(
        &self,
        context_key: &str,
        table_name: &str,
    ) -> Vec<ChangeEvent> {
    ...
    pub async fn clear_changes(&self, context_key: &str) {
        println!(
            "🧹 [Manager] clear_changes called for context: {}",
            context_key
        );
        let mut changes_map = self.changes.write().await;
        ...
        println!(
            "🧹 [Manager] Context exists: {}, Changes count: {}",
            had_changes, changes_count
        );

        changes_map.remove(context_key);

        println!(
            "🧹 [Manager] Context cleared. Remaining contexts: {}",
            changes_map.len()
        );
    }
    ...
    pub async fn clear_all_changes(&self) {
        ...
        println!(
            "💥 [Manager] Before clear - Contexts: {}, Total changes: {}",
            total_contexts, total_changes
        );
        ...
        println!(
            "💥 [Manager] After clear - Contexts: {}, Total changes: 0",
            changes_map.len()
        );
    }
    ...
    fn find_least_recently_used_context(
        &self,
        changes_map: &HashMap<String, VecDeque<ChangeEvent>>,
    ) -> Option<String> {
        changes_map
            .iter()
...
mod tests {
    use super::*;
    use crate::commands::database::change_history::types::{
        ChangeMetadata, OperationType, UserContext,
    };
    use uuid::Uuid;
    ...
    async fn test_memory_bounds_enforcement() {
        ...
        assert!(
            changes.len() <= 5,
            "Per-context limit not enforced: got {} changes",
            changes.len()
        );
        ...
        assert!(
            contexts.len() <= 3,
            "Total context limit not enforced: got {} contexts",
            contexts.len()
        );
    }

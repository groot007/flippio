 ...
 * @returns Mutation function for deleting table rows
 */
export function useDeleteRowMutation() {
  return useBaseDatabaseMutation<DeleteRowVariables, { selectedDatabaseFile: any, selectedDatabaseTable: any }>({
    mutationFn: async ({ selectedRow, selectedDatabaseTable, tableColumns, ...context }) => {
      ...
      const rowData = selectedRow?.rowData || selectedRow?.originalData
      if (!rowData) {
        throw new Error('No row data available for deletion')
      }
 ...
 * @returns Mutation function for clearing entire tables
 */

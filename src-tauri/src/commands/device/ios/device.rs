...
use super::super::types::{Device, DeviceResponse};
...
#[tauri::command]
pub async fn device_get_ios_devices(
...
pub async fn device_check_app_existence(
    ...
    let app_exists = apps_output.lines().enumerate().any(|(i, line)| {
        let contains = line.contains(&package_name);
        if contains {
            info!(
                "✅ Found package '{}' in line {}: '{}'",
                package_name,
                i + 1,
                line
            );
    ...
    info!(
        "App '{}' exists on device '{}': {}",
        package_name, device_id, app_exists
    );
...
#[tauri::command]
pub async fn ios_get_device_info(
    app_handle: tauri::AppHandle,
    device_id: String,
    ...
    match get_ios_device_detailed_info(&app_handle, &device_id).await {
        Ok(info) => {
            info!(
                "Successfully retrieved iOS device info with {} properties",
                info.len()
            );
            Ok(DeviceResponse {
                success: true,
...
async fn get_ios_device_detailed_info(
    app_handle: &tauri::AppHandle,
    device_id: &str,
    ...
    info!(
        "Device type detection - is_simulator: {}, device_id length: {}",
        is_simulator,
        device_id.len()
    );
...
async fn get_simulator_device_info(
    app_handle: &tauri::AppHandle,
    device_id: &str,
) -> Result<std::collections::HashMap<String, String>, Box<dyn std::error::Error + Send + Sync>> {
    info!(
        "Getting simulator device info using xcrun simctl for: {}",
        device_id
    );
                ...
                for device in device_array {
                    ...
                    if device_udid == device_id || device_id.contains(device_name) {
                        ...
                        if let Some(availability) =
                            device.get("availability").and_then(|a| a.as_str())
                        {
                            device_info
                                .insert("Availability".to_string(), availability.to_string());
                        }
    ...
    info!(
        "Successfully retrieved {} simulator properties",
        device_info.len()
    );
...
async fn get_physical_device_info(
    app_handle: &tauri::AppHandle,
    device_id: &str,
) -> Result<std::collections::HashMap<String, String>, Box<dyn std::error::Error + Send + Sync>> {
    info!("Getting detailed iOS device info for device: {}", device_id);
    ...
    for line in stdout.lines() {
        if let Some(colon_pos) = line.find(':') {
            ...
            match key {
                ...
                "BuildVersion" => {
                    device_info.insert("Build Version".to_string(), value.to_string());
                }
                "ModelNumber" => {
                    device_info.insert("Model Number".to_string(), value.to_string());
                }
                "CPUArchitecture" => {
                    device_info.insert("CPU Architecture".to_string(), value.to_string());
                }
                "HardwarePlatform" => {
                    device_info.insert("Hardware Platform".to_string(), value.to_string());
                }
                "SerialNumber" => {
                    device_info.insert("Serial Number".to_string(), value.to_string());
                }
                "UniqueDeviceID" => {
                    device_info.insert("UDID".to_string(), value.to_string());
                }
                "TotalDiskCapacity" => {
                    device_info.insert(
                        "Storage Capacity".to_string(),
                        format_bytes(value.parse().unwrap_or(0)),
                    );
                }
                "TotalSystemAvailable" => {
                    device_info.insert(
                        "Available Storage".to_string(),
                        format_bytes(value.parse().unwrap_or(0)),
                    );
                }
                _ => {}
    ...
    info!(
        "Processed {} lines from ideviceinfo output",
        processed_lines
    );
    ...
    info!(
        "Successfully retrieved {} iOS device properties",
        device_info.len()
    );

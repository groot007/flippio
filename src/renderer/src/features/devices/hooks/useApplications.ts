...
/**
 * Hook for fetching applications/packages from a selected device
 * @param selectedDevice - The device to fetch applications from
 * @returns Query result with applications list
 */
export function useApplications(selectedDevice: Device | null) {
  return useQuery({
    queryKey: selectedDevice?.id 
      ? deviceQueryKeys.applications(selectedDevice.id)
      : ['applications', null],
    queryFn: async () => {

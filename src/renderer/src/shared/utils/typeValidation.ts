...
export function validateFieldValue(value: any, type: string): ValidationResult {
  ...
  // Handle null and undefined consistently across all types
  ...
  // Handle empty strings consistently across all types - return null for database consistency
  if (stringValue === '') {
    return { isValid: true, convertedValue: null }
  }
  ...
  switch (normalizedType) {
    case 'INTEGER':
    case 'INT': {
      // Check if the string contains only digits, optional leading sign
